...
function PostForm({ post }) {
  ...
  const submit = async (data) => {
    if (post) {
      ...
      if (file) {
        appwriteService.delateFile(post.featuredImage);
      }
  ...
  return <form onSubmit={handleSubmit(submit)} className="flex flex-wrap">
    <div className="w-2/3 px-2">
        ...
        <Input label="Slug :" placeholder="Slug" className="mb-4" {...register("slug", { required: true })} onInput={(e) => { setValue("slug", slugTransform(e.target.value), { shouldValidate: true});}} />
